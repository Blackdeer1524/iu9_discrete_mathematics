...
using outputs_table     = std::vector<std::vector<std::string>>;
...
auto main() -> int {
    ...
    outputs_table phi(states_count, std::vector<std::string>(in_alphabet_size));
    ...
    std::vector<std::unordered_set<std::string>> z(states_count);
    ...
    for (uint64_t z_class_i = 0; z_class_i < a.states_count; ++z_class_i) {
        ...
        for (uint64_t in_signal_i = 0; in_signal_i < a.alphabet_size;
            ...
            const auto output           = a.phi.at(z_class_i).at(in_signal_i);
