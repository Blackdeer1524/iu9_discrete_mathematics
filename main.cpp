...
auto aufenkamp_hohn(const Automaton &A) -> Automaton {
    ...
    for (uint64_t i = 0; i < k_equivalent_count; ++i) {
        new_delta.at(i).equivalence_class = new SetItem;
    ...
    return {new_q0, new_delta, new_phi, k_equivalent_count, A.alphabet_size};
}
...
auto print_automator(const Automaton &a) -> void {
    ...
    for (uint64_t state_i = 0; state_i < a.states_count; ++state_i) {
        for (uint64_t in_signal_i = 0; in_signal_i < a.alphabet_size;
             ++in_signal_i) {
            const auto next_state =
                a.delta.at(state_i).transitions.at(in_signal_i);
            const auto output = a.phi.at(state_i).at(in_signal_i);
...
auto main() -> int {
    ...
    const Automaton a{q0, delta, phi, n, m};
    const Automaton minimized = aufenkamp_hohn(a);
